...
public class PrefabsTreeViews : TreeView
{
    ...
    private PrefabsTreeItem CreateItem(Transform transform, int id)
    {
        ...
        if (GenUIUtils.SelectedDic.ContainsKey(id))
        {
            item.defaultSelect = GenUIUtils.SelectedDic[id];
        }
        else
        {
            GenUIUtils.SelectedDic.Add(id, item.defaultSelect);
        }
        return item;
    }
    ...
    protected override void RowGUI(RowGUIArgs args)
    {
        ...
        {
            GenUIUtils.SelectedDic[item.id] = isStatic;
