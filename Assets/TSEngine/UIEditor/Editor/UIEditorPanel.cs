...
public class UIEditorPanel : EditorWindow
{
    ...
    {
        this.titleContent = new GUIContent("自动生成UI代码");
    }
    ...
    private List<GameObject> BuildSelectUIList()
    {
        ...
        if (this.uiCompleteList.Count == GenUIUtils.SelectedDic.Count)
        {
            for (int i = 0; i < GenUIUtils.SelectedDic.Count; i++)
            {
                if (GenUIUtils.SelectedDic[i] && this.uiCompleteList[i] != null)
                {
    ...
    private StringBuilder WriteCSUsing()
    {
        ...
        if (this.isCSCode)
        {
            ...
            usingBuilder.Append(UICodeConfig.namespaceRegionStart);
    ...
    private StringBuilder GenerateCode()
    {
        ...
        List<UICompleteEnum> list = GenUIUtils.UIComponentList(this.BuildSelectUIList());
